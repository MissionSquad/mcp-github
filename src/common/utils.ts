...
export async function checkUserExists(github_pat: string, username: string): Promise<boolean> {
  try {
    await githubRequest(github_pat, `https://api.github.com/users/${username}`);
    return true;
  } catch (error) {
    if (error && typeof error === "object" && "status" in error && error.status === 404) {
      return false;
